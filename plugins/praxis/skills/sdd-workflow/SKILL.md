...
3. **実装**
    - 実装計画に沿って実装する  
    - 引き継ぎメモ: `.praxis/agent-tasks/<ブランチ名>/<TOPIC>/handoff.md`
    - 完了後にコンテキストをクリアする
4. **確認・修正**
...
> [!WARNING]
> この時点でADRを作成する指示を入れてはいけない。意思決定は「4. 確認・修正」で変更されうる
...
> [!WARNING]
> この時点でADRを作成する手順を作ってはいけない。意思決定は「4. 確認・修正」で変更されうる
...
> [!WARNING]
> この時点でADRを作成してはいけない。意思決定は「4. 確認・修正」で変更されうる
...
実装が完了したら「4. 確認・修正」の前にコンテキストをクリアする。実装セッションの会話はここで失われるため、**会話の中だけに存在する情報を `handoff.md` に書き出す。**

`spec.md` `plan.md` とgit履歴はクリア後も読めるので重複して書かない。書くのは次の5点に限る。

- **完了状態** : `plan.md` のどのタスクが完了し、どれが未完了か
- **計画からの逸脱** : `plan.md` と異なる実装をした箇所と、そうした理由
- **AI独自の判断** : 仕様に明記がなくAIが決めた箇所
- **未確認・リスク** : 動作確認できていない箇所、仕様上うまく動作しないリスクが高い箇所
- **確認手順** : ユーザーが動作確認するためのコマンドと手順

### 3.4 コンテキストのクリア

`handoff.md` を書き終えたら、ユーザーに次の2つを提示する。コンテキストのクリアはユーザーが実行する。

1. コンテキストをクリアするコマンド。Claude Code・Codexとも `/clear` で新しい会話を開始できる ([Claude Code](https://code.claude.com/docs/en/interactive-mode) / [Codex](https://developers.openai.com/codex/cli/slash-commands))

```text
/clear
```

2. クリア後の最初のプロンプトとして貼り付ける文面。スキルの呼び出し方はホストによって異なるため利用中のホストに合わせ、`<ブランチ名>` `<TOPIC>` は実際の値に置き換えて提示する。

````text
/praxis:sdd-workflow の「4. 確認・修正」を開始する。
まず「4.0 状況の把握」に従って状況を把握すること。
対象タスク: `.praxis/agent-tasks/<ブランチ名>/<TOPIC>/`
````

Codex利用時は1行目を `$sdd-workflow の「4. 確認・修正」を開始する。` に置き換える。
...
このフェーズはコンテキストのクリア後に始まるため、実装の経緯を持たない状態からスタートする。**最初のユーザー発言に対応する前に、次の順で状況を把握する。**

以下の順序で確認すること:
1. `spec.md` — あるべき姿。全文を読む
2. `plan.md` — 実装計画とタスクの完了状態
3. `handoff.md` — 実装結果、計画からの逸脱、AI独自の判断、残リスク
4. `fixes.md` — 既存の修正記録。存在する場合のみ

状況を把握し終えたらユーザーの発言を待つ。
...
| `handoff` | 更新しない。実装完了時点のスナップショットとして保持する |
